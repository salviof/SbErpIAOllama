package org.coletivoJava.fw.projetos.erpColetivoJava.implementacao.cucumber.fluxoollama.etapas;

import org.coletivoJava.fw.projetos.erpColetivoJava.api.cucumber.fluxoollama.EtapasFluxoOllama;
import cucumber.api.java.pt.Quando;
import java.lang.UnsupportedOperationException;

public class Quando_o_usuario_envia_a_mensagem_perguntando_nome_e_idade_para_persona_joao {

	@Quando(EtapasFluxoOllama.QUANDO_O_USUARIO_ENVIA_A_MENSAGEM_PERGUNTANDO_NOME_E_IDADE_PARA_PERSONA_JOAO)
	public void implementacaoEtapa() {
		throw new UnsupportedOperationException(
				"Etapa 'o usuario envia a mensagem perguntando nome e idade para persona joao' não implementadas");
	}
}